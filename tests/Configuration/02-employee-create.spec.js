...
test('test', async ({ page }) => {
  ...
  .fill(`emp${Math.floor(Math.random() * 10000)}`);

  await page.locator('.css-18w4uv4').first().click();
  await page.getByRole('option', { name: 'Senior Developmer' }).click();
  ...
  await page.getByText('EmailJoining Date*').click();
  ...
  await page.getByPlaceholder('NID')
  .fill(`${Math.floor(1e16 + Math.random() * 9e16)}`);
